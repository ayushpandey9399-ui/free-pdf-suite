...
// Lazy WASM encoder (Safari + older Firefox lack canvas WebP encoding).
...
async function jpgToWebp(file: File, quality: number): Promise<Blob> {
  ...
  // Try native canvas WebP first.
  const nativeBlob = await canvasEncodeWebp(canvas, quality);
  if (nativeBlob && nativeBlob.type === "image/webp") {
    return nativeBlob;
  }

  // Fallback: WASM encoder (Safari, some Firefox). Never return a PNG named .webp.
...
export function JpgToWebpTool() {
  ...
  const convertAll = async () => {
    ...
    for (const row of rows) {
      ...
      try {
        const blob = await jpgToWebp(row.file, quality);
        const base = row.file.name.replace(/\.jpe?g$/i, "");
  ...
  const downloadZip = async () => {
    ...
    for (const r of done) {
      const base = r.file.name.replace(/\.jpe?g$/i, "");
      zip.file(uniqueZipName(used, `${base}.webp`), r.outBlob!);
    }
    const blob = await zip.generateAsync({ type: "blob" });
    saveAs(blob, "jpg-to-webp.zip");
  };
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      >
        ...
        <p className="mt-3 text-[13px] text-[#5a5a66]">or drop .jpg images here</p>
