...
async function webpToPng(file: File): Promise<Blob> {
  ...
  canvas.width = w;
  canvas.height = h;
  // Keep alpha channel. Do NOT pass { alpha: false } and do NOT fill any
  // background color. Transparent pixels in the WebP stay transparent in
  // the resulting PNG.
...
export function WebpToPngTool() {
  ...
  const convertAll = async () => {
    ...
    for (const row of rows) {
      ...
      try {
        const blob = await webpToPng(row.file);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      {rows.length > 0 && (
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3 rounded-xl border border-[#ececef] bg-white p-4">
            <span className="text-[13px] text-[#5a5a66]">
              PNG is lossless. Transparency is preserved.
            </span>
            ...
            {rows.map((r) => (
              ...
              >
                <div
                  ...
                  style={{
                    // Checkerboard so preserved transparency is visible
                    backgroundImage:
                ...
                >
                  {r.previewUrl ? (
                    <img
                      src={r.previewUrl}
                      alt={`Converted PNG preview of ${r.file.name} (transparency preserved)`}
