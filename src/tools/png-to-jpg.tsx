...
async function pngToJpg(file: File, quality: number): Promise<Blob> {
  ...
  // CRITICAL: JPG has no alpha, fill white first so transparent pixels
  // become white and not black.
...
export function PngToJpgTool() {
  ...
  const convertAll = async () => {
    ...
    for (const row of rows) {
      ...
      try {
        const blob = await pngToJpg(row.file, quality);
