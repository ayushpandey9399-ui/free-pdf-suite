...
// Internal components exported for lazy loading
export function PrivacyStoryInternal() {
  return (
...
export function WhyChooseInternal() {
  return (
...
export function HowItWorksInternal() {
  return (
    <section style={{ backgroundColor: BAND }} className="py-14 sm:py-24">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
        <RevealInternal>
          ...
          <div className="relative mt-14">
            <div
              ...
              style={{
                height: 1,
                backgroundImage: "linear-gradient(to right, #D1D5DB 50%, transparent 0%)",
                ...
                <div key={s.n} className="flex flex-col items-center text-center">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: "0 0 0 6px rgba(229,50,45,0.10), 0 6px 14px -4px rgba(229,50,45,0.4)",
                    }}
                  >
                    {s.n}
                  </div>
...
export function SeoProseInternal() {
  return (
    <section className="py-14 sm:py-24">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
        <RevealInternal>
          ...
          <div className="mx-auto mt-8 max-w-[760px] text-left text-[16px]" style={{ color: MUTED, lineHeight: 1.7 }}>
            <p>
              ...
              reordering or deleting pages. Convert <SeoLink slug="images-to-pdf">images to PDF</SeoLink> and PDF
              pages to images, pull the text out of any document, or turn plain
              text files into clean PDFs. Edit and annotate, <SeoLink slug="fill-forms">fill out forms</SeoLink>, add page
              numbers, headers and watermarks, all without installing anything.
            </p>
            <p className="mt-5">
              When documents get sensitive, <SeoLink slug="protect-pdf">protect them with a password</SeoLink>,{" "}
...
export function FinalCtaInternal() {
...
// Lazy components pointing to internal exports
const PrivacyStory = lazy(() => import("./HomeBottom").then(m => ({ default: m.PrivacyStoryInternal })));
const WhyChoose = lazy(() => import("./HomeBottom").then(m => ({ default: m.WhyChooseInternal })));
const HowItWorks = lazy(() => import("./HomeBottom").then(m => ({ default: m.HowItWorksInternal })));
const SeoProse = lazy(() => import("./HomeBottom").then(m => ({ default: m.SeoProseInternal })));
const FinalCta = lazy(() => import("./HomeBottom").then(m => ({ default: m.FinalCtaInternal })));
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 32,
        boxShadow: "0 1px 2px rgba(20,20,43,0.04), 0 12px 32px -18px rgba(20,20,43,0.18)",
      }}
    >
      <div className="flex flex-col items-center">
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <span className="text-[13px]" style={{ color: MUTED }}>Processed on your device</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ...
        style={{ backgroundColor: "#F3F4F6", border: `1px solid ${BORDER}` }}
      />
      <span className="mt-2 text-[11px] font-medium" style={{ color: MUTED }}>{label}</span>
    </div>
  );
}

const REASONS = [
  { icon: ShieldCheck, title: "100% private", body: "Our tools run in your browser. Where server processing is used, files are securely deleted immediately." },
  { icon: Gift, title: "Completely free", body: "All 44 tools with no signup, no watermarks, no daily limits and no premium wall." },
  { icon: Zap, title: "Fast on any device", body: "No upload queues, no waiting. Everything runs instantly on your phone, tablet or computer." },
  { icon: WifiOff, title: "Works even offline", body: "Once a tool page has loaded, it keeps working without an internet connection." },
];

function ReasonCard({ Icon, title, body }: { Icon: any; title: string; body: string }) {
  const [hover, setHover] = useState(false);
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hover ? "0 10px 24px -14px rgba(20,20,43,0.16)" : "0 1px 2px rgba(20,20,43,0.03)",
      }}
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
    >
...
const STEPS = [
  { n: 1, title: "Pick a tool", body: "Choose any of the 44 tools above, from merging and compressing to signing and redacting." },
  { n: 2, title: "Add your file", body: "Drag and drop or tap to select. The file opens right in your browser and stays on your device." },
  { n: 3, title: "Download the result", body: "Get your finished PDF instantly, clean output, no watermark, no signup screen." },
];

function SeoLink({ slug, children }: { slug: string; children: React.ReactNode }) {
  return (
    <Link to="/tools/$slug" params={{ slug }} className="underline decoration-[#F3B4B2] decoration-2 underline-offset-4 transition-colors duration-150 hover:text-[#E5322D]" style={{ color: INK }}>
      {children}
    </Link>
  );
}
